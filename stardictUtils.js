...
function get_chunks(gzip_header)
{
	...
	var ver, chlen = 0, chcnt = 0, chunks = [];
...
function getArtInfo(pos, len, ab, gzip_header, vccc)
{
	...
	ArtInfo[0] = [gzip_header["LENGTH"] + chunks[firstchunk][0], gzip_header["LENGTH"] + chunks[lastchunk][0] + chunks[lastchunk][1]];
	ArtInfo[1] = []
	ArtInfo[2] = [offset, finish];

	let in_buffer = ab.slice(gzip_header["LENGTH"] + chunks[firstchunk][0],  gzip_header["LENGTH"] + chunks[lastchunk][0] + chunks[lastchunk][1]);

	for(var i = firstchunk, j = 0;  i <=  lastchunk && j < in_buffer.byteLength; j += chunks[i][1], i++)
	{
...
function getSliceChunksTable(dzfile, OffsetLengthTable)
{
	...
	{
		let ArtInfo = getArtInfo(OffsetLengthTable[i][1], OffsetLengthTable[i][2], arraybuffer, gzip_header, vccc);
...
function getArticleBodyfromDZ2(dzfile, ArtInfo)
{
	...
	*/
	...
	const bigchunk = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
	...
	for(let i = 0; i < ArtInfo[1].length; i++)
	{
		...
		let inflated = zlib.inflateRawSync(chunk, { finishFlush: zlib.constants.Z_SYNC_FLUSH });
		out_buffer = mergeArrayBuffers(out_buffer, inflated);
		
	}

	return Buffer.from(out_buffer.slice(ArtInfo[2][0], ArtInfo[2][1])).toString('utf8');
